...
class PAMAP2(Dataset, ProfileMixin):
    logger = getLogger('datasets.pamap2.PAMAP2')
    ...
    def _cache_load(cls, raw_dir: Text, processed_dir: Text, fname: Text) -> np.ndarray:
        ...
        cache_path = path.join(processed_dir, fname.replace('.dat', '.raw.npy'))
        raw_path = path.join(raw_dir, fname)
        if path.exists(cache_path) and path.getsize(cache_path) > 0:
            return np.load(cache_path)
        else:
            class_idx = [i for i in range(-1, 12)]
            temp = pd.read_csv(raw_path, header=None, encoding='utf-8', delimiter=' ', dtype='float32')
            temp = np.array(temp)
            raw_array = np.concatenate((temp[:, 4:16], temp[:, 21:33], temp[:, 38:50], temp[:, 1][:, np.newaxis]),
                                       axis=1)
    ...
    def _preprocess_robust(cls, array: np.ndarray) -> np.ndarray:
        ...
        return preprocessing.robust_scale(scaled_array, axis=1).reshape(channels, frames, window_size).transpose(1, 0,
                                                                                                                 2)
    ...
    def _preprocess_minmax(cls, array: np.ndarray) -> np.ndarray:
        ...
        scaled_array = array.transpose((1, 0, 2)).reshape((channels, -1))
        scaled_array = preprocessing.robust_scale(scaled_array, axis=1)
        return preprocessing.minmax_scale(scaled_array, axis=1).reshape(channels, frames, window_size).transpose(1, 0,
                                                                                                                 2)
    ...
    def __init__(self, **kwargs):
        ...
        for it in filter_by:
            assert it in tuple(pool_map(lambda idx: f'subject{idx}', range(101, 110)))
        ...
        raw_dir = path.join(root_dir, 'Protocol')
        ...
        raw_subjects = dict(pool_map(
            lambda it: (it[:-4], self._cache_load(raw_dir, processed_dir, it)),
            filter(
                lambda it: it.endswith('.dat'),
                listdir(raw_dir)
            )
        ))

...
class SqueezeExcitationSoftmaxBlock(SqueezeExcitationBlock):
    def forward(self, *input: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        return super(SqueezeExcitationSoftmaxBlock, self).forward(*input, **kwargs).softmax(dim=1)


class FakeAttentionBlock(nn.Module):
    def __init__(self, *args, **kwargs):
        super(FakeAttentionBlock, self).__init__()
